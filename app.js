...
const ACADEMIC_DATA = {
  ...
  10: {
    ...
    chapters: [
      {
        num: 1,
        title: "Methods of Communication with Part A All answer",
        desc: "Methods of Communication",
      ...
      {
        num: 2,
        title: "Communication Cycle with Part B All answer",
        desc: "Communication Cycle",
        answerKeyUrl: "https://drive.google.com/file/d/1P24Pt1ipBJF8xcNe7S7vNoI5EVHb6Yfe/view?usp=drive_link", // TODO: Add Class 10 Chapter 2 Answer Key Drive Link
  ...
  "maths": {
    className: "Maths",
    description: "Access Mathematics resources, reference sheets, and homework materials.",
    iconColor: "var(--color-accent)",
    isMaths: true,
    chapters: []
  }
...
const ICONS = {
  ...
  mathsIcon: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-hash"><line x1="4" y1="9" x2="20" y2="9"></line><line x1="4" y1="15" x2="20" y2="15"></line><line x1="10" y1="3" x2="8" y2="21"></line><line x1="16" y1="3" x2="14" y2="21"></line></svg>`,
...
function initClassCards() {
  ...
  Object.keys(ACADEMIC_DATA).forEach((classId) => {
    ...
    const icon = data.isMaths ? ICONS.mathsIcon : ICONS.classIcon;
    const label = data.isMaths ? "3 Study Resources" : `${data.chapters.length} Computer Chapters`;
...
function renderWorkspaceHeader() {
  ...
  if (title) {
    title.innerText = data.isMaths ? `Mathematics Coursework` : `Computer Science Coursework`;
  }
}
...
function renderChapters() {
  ...
  // Render mathematics resource gallery
  if (classData.isMaths) {
    chaptersContainer.innerHTML = "";
    
    const galleryEl = document.createElement("div");
    galleryEl.classList.add("maths-gallery");
    
    const mathsImages = [
      {
        src: "media/maths/WhatsApp Image 2026-08-25 at 07.34.19.jpeg",
        title: "Mathematics Lesson Sheet 1",
        desc: "First reference diagram showing algebraic equations and geometry calculations."
      },
      {
        src: "media/maths/WhatsApp Image 2026-08-25 at 07.34.47.jpeg",
        title: "Mathematics Lesson Sheet 2",
        desc: "Second reference diagram presenting math graphs, coordinate planes, and formulas."
      },
      {
        src: "media/maths/WhatsApp Image 2026-08-25 at 07.34.57.jpeg",
        title: "Mathematics Lesson Sheet 3",
        desc: "Third reference sheet with arithmetic steps, worksheets, and practice questions."
      }
    ];

    mathsImages.forEach((img, idx) => {
      const card = document.createElement("div");
      card.classList.add("maths-card");
      card.style.animation = `slide-up 0.4s ease-out ${idx * 0.1}s both`;
      
      card.innerHTML = `
        <div class="maths-card-preview" onclick="openImageFullscreen('${img.src}', '${img.title}')">
          <img src="${img.src}" alt="${img.title}" loading="lazy">
          <div class="maths-card-hover-overlay">
        ...
        <div class="maths-card-info">
          <h4>${img.title}</h4>
          <p>${img.desc}</p>
          <div class="maths-card-actions">
            <button class="btn btn-primary" onclick="openImageFullscreen('${img.src}', '${img.title}')">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line><line x1="11" y1="8" x2="11" y2="14"></line><line x1="8" y1="11" x2="14" y2="11"></line>
              </svg>
              <span>View Fullscreen</span>
            </button>
            <a href="${img.src}" target="_blank" class="btn btn-secondary">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    chaptersContainer.appendChild(galleryEl);
    return;
  }
...
// Inline image viewer overlay presentation handler
function openImageFullscreen(src, title) {
  const modal = document.getElementById("image-modal");
  const img = document.getElementById("image-modal-img");
  const modalTitle = document.getElementById("image-modal-title");

  if (!modal || !img || !modalTitle) return;
  ...
  function closeImageModal() {
    modal.classList.remove("active");
    modal.setAttribute("aria-hidden", "true");
    img.src = ""; // Clear source to free memory/avoid flash on next open
    window.removeEventListener("keydown", handleKeydown);
    modal.removeEventListener("click", handleBgClick);
    if (closeBtn) closeBtn.removeEventListener("click", handleCloseClick);
  }
